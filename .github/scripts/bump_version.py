...
release section in `CHANGELOG.md` (Keep a Changelog format). discord-vexa-bridge
ships as a Docker image / GitHub release rather than a PyPI package, so there
is no `.mike.yml` or `versions.json` to maintain — only these two files.
