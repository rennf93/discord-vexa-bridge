...
### Added
- **`meeting.completed` webhook emission.** This bridge writes Discord meetings straight into Vexa's Postgres, so Vexa's own webhook emitter never sees them and `meeting.completed` never fires for a Discord call. Set `COMPLETION_WEBHOOK_URL` (and the now-required `COMPLETION_WEBHOOK_SECRET`) to have `_finalize_completed` emit a `meeting.completed` event once a meeting's pending transcription queue has fully drained, as a fire-and-forget background task: `event_id`, `event_type`, `api_version`, `created_at`, and a `data.meeting` object with `id`, `platform`, `native_meeting_id`, `status`, `completion_reason`, `start_time`, `end_time`, and a `source` marker, signed the same way Vexa signs its own webhooks (`sha256=hmac(secret, timestamp + "." + body)`). This is not Vexa's full `webhook.v1` envelope: its `data.meeting` also carries `user_id`, `constructed_meeting_url`, `failure_stage`, `data`, `created_at`, and `updated_at`, and this bridge's `event_id` hash input is its own, not Vexa's. `event_id` is deterministic per meeting so redeliveries dedup on the receiving end. New `completion_webhook.py` module (`build_envelope`, `sign`, `headers_for`, `emit`); unset URL keeps the feature off. The reference receiver is [`obsidian-vexa-bridge`](https://github.com/rennf93/obsidian-vexa-bridge).

### Fixed
- **Completion webhook tolerates missing timestamps.** `start_time` and `end_time` are both nullable in Vexa's schema; a still-open or never-started row used to raise inside `_finalize_completed` and drop that meeting's webhook for good. `build_envelope` now omits `end_time` when it is null and falls `start_time` back to `end_time`, then to `created_at`, and the call site wraps envelope building in `try`/`except` so a bad row only costs the webhook, not the meeting's completion. Replaced the `assert`-based URL/secret guard in `_emit_completion_webhook` with an explicit check.
